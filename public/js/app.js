import {
	renderSelectedSectionandHeader,
	renderInitialAppState,
	renderFooterContent,
} from "./domutils.js"
import { superelevationConversionFromMilimetersToDegrees } from "./functions/engineering_functions.js"
import {
	calculateSuperelevation,
	superelevationConvert,
} from "./components/superelevation.js"
import {
	calculateTCParameters,
	divideTC,
	computeMinimalLengthTC,
	calculateTCforCurves,
	computeMinimalLengthTCForCurves,
} from "./components/transitioncurves.js"
import {
	calculateWideningFirstMethod,
	calculateWideningSecondMethod,
	calculateWideningThirdMethod,
	calculateWideningOnCurve,
} from "./components/widenings.js"
import {
	minimalLength,
	computeCurveMinimalRadius,
	calculateMinimalCurveRadiusWithoutTC,
	computeAngle,
	computeLengthFromSlant,
	computeLengthFromAngle,
} from "./components/turns.js"
import {
	verticalInclinesFirstMethod,
	verticalInclinesSecondMethod,
	convertHeightDifferenceToAngle,
	minimalVerticalCurveRadius,
} from "./components/verticalcurves.js"
...
const superelevationEventHandler = () => {
	const curveParameters = {
		...
		"isPlatform":
			document.querySelector("#superelevation_is_platform").value === "true"
				? true
				: false,
	...
	const { Dmin, Dreq, Dmed, Deq, Dmax } =
		calculateSuperelevation(curveParameters)
	...
	document.querySelector("#dmin_result_in_degrees").value =
		superelevationConversionFromMilimetersToDegrees(Dmin)
	document.querySelector("#dreq_result_in_milimeters").value = Dreq
	document.querySelector("#dreq_result_in_degrees").value =
		superelevationConversionFromMilimetersToDegrees(Dreq)
	document.querySelector("#dmed_result_in_milimeters").value = Dmed
	document.querySelector("#dmed_result_in_degrees").value =
		superelevationConversionFromMilimetersToDegrees(Dmed)
	document.querySelector("#deq_result_in_milimeters").value = Deq
	document.querySelector("#deq_result_in_degrees").value =
		superelevationConversionFromMilimetersToDegrees(Deq)
	document.querySelector("#dmax_result_in_milimeters").value = Dmax
	document.querySelector("#dmax_result_in_degrees").value =
		superelevationConversionFromMilimetersToDegrees(Dmax)
}

document
	.querySelector("#superelevation_compute_input")
	.addEventListener("input", superelevationEventHandler)
document
	.querySelector("#superelevation_calctype")
	.addEventListener("input", superelevationEventHandler)
...
document
	.querySelector("#tc_parameters_input")
	.addEventListener("input", tcParametersEventHandler)
...
document
	.querySelector("#tc_division_input")
	.addEventListener("input", tcDivisionEventHandler)
document
	.querySelector("#tc_parameters_input")
	.addEventListener("input", tcDivisionEventHandler)
...
const superelevationConvertEventHandler = () => {
	const superelevationInMilimeters = Number(
		document.querySelector("#superelevation_in_milimeters").value
	)
	const superelevationInDegrees = Number(
		document.querySelector("#superelevation_in_degrees").value
	)
	const superelevationCase = document.querySelector(
		"#superelevation_case"
	).value
	...
	const superelevationToConvert =
		superelevationCase === "degrees"
			? superelevationInMilimeters
			: superelevationInDegrees
	const superelevationResult = superelevationConvert(
...
document
	.querySelector("#superelevation_case")
	.addEventListener("change", superelevationCaseRender)
document
	.querySelector("#superelevation_conversion_input")
	.addEventListener("input", superelevationConvertEventHandler)
...
const minimalTCLengthEventHandler = () => {
	const curveParametersInput = {
		...
		superelevation: Number(
			document.querySelector("#minimal_tc_length_superelevation").value
		),
	...
	const { length, halflength, shift } =
		computeMinimalLengthTC(curveParametersInput)
...
document
	.querySelector("#minimal_tc_length_input")
	.addEventListener("input", minimalTCLengthEventHandler)
...
const minimalLengthCurveTCEventHandler = () => {
	const currentCase = document.querySelector(
		"#minimal_tc_length_curve_case"
	).value
	const currentState = currentCase === "1kp" ? "joint" : "separated"

	const firstArcParameters = {
		"radius": Number(
			document.querySelector("#minimal_tc_length_curve_radius_1").value
		),
		"direction": document.querySelector("#minimal_tc_length_curve_direction_1")
			.value,
		"vmax": Number(
			document.querySelector("#minimal_tc_length_curve_velocity").value
		),
		"superelevation": Number(
			document.querySelector("#minimal_tc_length_curve_superelevation_1").value
		),
		"curvetype": document.querySelector("#minimal_tc_length_curve_type").value,
		"calctype": document.querySelector("#minimal_tc_length_curve_valuelist")
			.value,
	}
	const {
		radius: firstCurveRadius,
		vmax,
		direction: firstCurveDirection,
	} = firstArcParameters

	const secondArcParameters = {
		"radius": Number(
			document.querySelector("#minimal_tc_length_curve_radius_2").value
		),
		"direction": document.querySelector("#minimal_tc_length_curve_direction_2")
			.value,
		"vmax": Number(
			document.querySelector("#minimal_tc_length_curve_velocity").value
		),
		"superelevation": Number(
			document.querySelector("#minimal_tc_length_curve_superelevation_2").value
		),
		"curvetype": document.querySelector("#minimal_tc_length_curve_type").value,
		"calctype": document.querySelector("#minimal_tc_length_curve_valuelist")
			.value,
	}
	const { radius: secondCurveRadius, direction: secondCurveDirection } =
		secondArcParameters

	if ([firstCurveRadius, secondCurveRadius, vmax].includes(0)) {
		document
			.querySelector("#minimal_tc_length_curve_output")
	...
	if (currentState === "separated") {
		const {
			"length": firstCurveLength,
			"halflength": firstCurveHalflength,
			"shift": firstCurveShift,
		} = computeMinimalLengthTC(firstArcParameters)
		const {
			"length": secondCurveLength,
			"halflength": secondCurveHalflength,
			"shift": secondCurveShift,
		} = computeMinimalLengthTC(secondArcParameters)
		document.querySelector("#minimal_tc_curve_length_1").value =
			firstCurveLength
		document.querySelector("#minimal_tc_curve_halflength_1").value =
			firstCurveHalflength
		document.querySelector("#minimal_tc_curve_shift_1").value = firstCurveShift
		document.querySelector("#minimal_tc_curve_length_2").value =
			secondCurveLength
		document.querySelector("#minimal_tc_curve_halflength_2").value =
			secondCurveHalflength
		document.querySelector("#minimal_tc_curve_shift_2").value = secondCurveShift
	} else if (currentState === "joint") {
		if (firstCurveDirection !== secondCurveDirection) {
			document
				.querySelector("#minimal_tc_length_curve_output")
				.querySelectorAll("input")
				.forEach((input) => (input.value = ""))
			return alert(
				"Nie jest możliwe wykonanie takiej krzywej dla różnych kierunków łuku"
			)
		}
		const {
			l: length,
			n: shift,
			l2: halflength,
		} = computeMinimalLengthTCForCurves(firstArcParameters, secondArcParameters)
		document.querySelector("#minimal_tc_curve_length_joint").value = length
		document.querySelector("#minimal_tc_curve_halflength_joint").value =
			halflength
...
;[
	...
	item.addEventListener("input", minimalLengthCurveTCEventHandler)
)

document
	.querySelector("#curve_case")
	.addEventListener("change", CurveTCInputsVisibility)
...
const curveTCEventHandler = () => {
	...
	const radiusFirstCurve = Number(
		firstCurveElement.querySelector("input").value
	)
	const radiusSecondCurve = Number(
		secondCurveElement.querySelector("input").value
	)
	const curvecase = document.querySelector("#curve_case").value
	const lengths =
		curvecase === "1kp"
			? Number(document.querySelector("#curve_length_joint").value)
			: [
					Number(document.querySelector("#curve_length_1").value),
					Number(document.querySelector("#curve_length_2").value),
			  ]
	...
	if (
		!radiusFirstCurve ||
		!radiusSecondCurve ||
		(Array.isArray(lengths) ? lengths.includes(0) : lengths === 0)
	) {
	...
	const result = calculateTCforCurves(
...
curveInputsArray.forEach((element) =>
	element.addEventListener("input", curveTCEventHandler)
)
...
const minimalLengthCurveTCInputsVisibility = () => {
	...
	document
		.querySelectorAll("[data-arc-tc-minimal-length-curvetype]")
		.forEach((element) => {
			if (
				element.getAttribute("data-arc-tc-minimal-length-curvetype") ===
				currentState
			) {
				element.classList.remove("invisible")
			} else {
...
document
	.querySelector("#minimal_tc_length_curve_case")
	.addEventListener("change", minimalLengthCurveTCInputsVisibility)
...
const wideningFirstMethodEventHandler = () => {
	...
	const {
		...
		CurveParametersForWideningWithTwoCurvesAndInset
	)
...
document
	.querySelector("#compute_first_widening_method_parameters_input")
	.addEventListener("input", wideningFirstMethodEventHandler)
...
const wideningSecondMethodEventHandler = () => {
	...
	const { length, angle, curveLength, shift } = calculateWideningSecondMethod(
		curvesForWideningParameters
	)
...
document
	.querySelector("#compute_second_widening_method_parameters_input")
	.addEventListener("input", wideningSecondMethodEventHandler)
...
const wideningThirdMethodEventHandler = () => {
	...
	const { wideningLength, curveLength, shift } = calculateWideningThirdMethod(
		tcsForWideningParameters
	)
...
document
	.querySelector("#compute_third_widening_method_parameters_input")
	.addEventListener("input", wideningThirdMethodEventHandler)
...
const wideningOnCurveEventHandler = () => {
	...
	const {
		...
		firstCurveShift,
		secondCurveShift,
	} = calculateWideningOnCurve(curvesParametersForWidening)
...
document
	.querySelector("#compute_fourth_widening_method_parameters_input")
	.addEventListener("input", wideningOnCurveEventHandler)
...
document
	.querySelector("#vertical_case")
	.addEventListener("change", verticalCurveInputsVisibilityState)

const minimalVerticalCuvreRadiusEventHander = () => {
	const trackParameters = {
		"velocity": Number(
			document.querySelector("#minimal_vertical_radius_velocity").value
		),
		"tracktype": document.querySelector("#minimal_vertical_radius_tracktype")
			.value,
		"calctype": document.querySelector("#minimal_vertical_radius_caltype")
			.value,
	}
...
document
	.querySelector("#minimal_vertical_curve_radius_input")
	.addEventListener("input", minimalVerticalCuvreRadiusEventHander)
...
document
	.querySelector("#vertical_inclines_1_input")
	.addEventListener("input", verticalInclinesFirstMethodEventHandler)
...
document
	.querySelector("#vertical_inclines_2_input")
	.addEventListener("input", verticalInclinesSecondMethodEventHandler)
...
document
	.querySelector("#convert_height_difference_to_angle_input")
	.addEventListener("input", convertHeightDifferenceToAngleEventHandler)
...
document
	.querySelector("#turn_lenght_compute_input")
	.addEventListener("input", turnLength)
...
document
	.querySelector("#compute_minimal_radius_input")
	.addEventListener("input", computeCurveMinimalRadiusEventHandler)
...
const minimalRadiusWithoutTCEventHandler = () => {
	...
	document.querySelector("#turnwithouttc_rmin_result").value =
		minimalRadius != Infinity ? minimalRadius : "Łuk niemożliwy"
}

document
	.querySelector("#turnwithouttc_pamrameters")
	.addEventListener("input", minimalRadiusWithoutTCEventHandler)
...
document
	.querySelector("#turn_parameters_input")
	.addEventListener("input", turnparams)