import { cround, degrees } from "./math_functions.js"

export const superelevationConversionFromMilimetersToDegrees = (
    superelevationInMilimeters
  ) => cround(degrees(Math.atan(superelevationInMilimeters / 1435)), 1);
  ...
  export const calculateShiftforJointTC = (r1, r2, l, curvetype) => {
    const shiftPrototype = {
  ...
  export const DmaxValueList = (DmaxType, calctype, radius) => {
    ...
    if (["small curve radius", "extra small curve radius"].includes(DmaxType)) {
      return DmaxValueList[DmaxType][calctypeIndex] > 0
        ? Math.min(DmaxValueList[DmaxType][calctypeIndex], (radius - 50) / 1.5)
        : 0;
    }
  ...
  export const deltaIdopValueList = (calctype, tracktype, vmax) => {
    const calctypeIndex = calctypeToIndex(calctype);
